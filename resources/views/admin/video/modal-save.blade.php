<x-dialog-save-modal action="save" wire:model.live="flagOpenModal">
    ...
    <x-slot name="content">
        <div>
            ...
            <x-input wire:model.defer="video.label" id="label" class="block mt-1 w-full" type="text"
                     :value="old('user.label')" autofocus autocomplete="label"/>
            <x-input-error for="video.label" />
        </div>
        <div>
            <x-label for="url" value="{{ __('Url') }}"/>
            <x-input wire:model.defer="video.url" id="url" class="block mt-1 w-full" type="text"
